...
int verbose = 1,verify = 1,program = 1,sleep_time = 0,readflash = 0,getmcuids = 0;
int baudRate = 115200;
int devid_expected,devid_mask,com,flash_size,page_size,chip_family,config_size;
...
void printHelp()
    ...
    flsprintf(stdout,"   -p                    do not program\n");
...
int main(int argc, char *argv[])
    ...
    hex_ok = parse_hex(filename,pm_point,cm_point);					//parse and write content of hex file into buffers
...
//    for (i=0; i<10; i++) printf ("%2.2x",config_bytes[i]);
    ...
    if ((chip_family==CF_P18F_A)|(chip_family==CF_P18F_B)|(chip_family==CF_P18F_D)|(chip_family==CF_P18F_E)|(chip_family==CF_P18F_F)|(chip_family==CF_P18F_Q))
		...
		if (readflash == 1) {
				// TODO
				if (verbose>0) printf ("Reading FLASH (%d B in %d pages)",flash_size,flash_size/page_size);
				fflush(stdout);
				if ((chip_family==CF_P16F_A)|(chip_family==CF_P16F_B)|(chip_family==CF_P16F_D)) p16a_rst_pointer();
