...
device_cmds_t device_cmds[] = {
	{p16a_mass_erase, p16a_rst_pointer, NO_COMMAND, p16a_program_config}, // CF_P16F_A, 12f18xx
	...
	{p16a_mass_erase, p16a_rst_pointer, NO_COMMAND, p16a_program_config}, // CF_P16F_D, 16f3x6
...
int p16a_rst_pointer (void)
    ...
    getByte(0);						//return result - no check for its value
    return 0;
    }
...
int p16d_set_pointer (unsigned int addr)
	...
	putByte(addr & 0xff);           // LSB
	putByte((addr >> 8) & 0xff);    // MSB
    getByte(0);						//return result - no check for its value
    return 0;
    }
...
int p16a_mass_erase(void) {
    ...
    // if (verbose>2) flsprintf(stdout,"Waiting for mass erase to complete...\n");
    // sleep_ms(50);
    getByte(0x87);
    return 0;
}
...
int p16a_program_page(unsigned int ptr, unsigned char num, unsigned char slow) {
	...
	//
	// empty check but not for config
	//
	if (ptr >> 1 != 0x8007) {
    ...
    putByte(0x08);
    putByte(num + 2); // not used anywhere
    putByte(num);
    putByte(slow);
    /*
    for (i=0;i<num;i++)
    ...
    // need some waiting for operation to complete
    getByte(0x88);
    return 0;
}
...
int p16a_read_page (unsigned char *data, unsigned char num) {
    ...
    putByte(num/2);
    getByte(0);
    for (i=0; i<num; i++) *data++ = getByte(0);
    return 0;
}
...
int p16a_read_eeprom (unsigned char *data, unsigned char num) {
    ...
    cmd = 0x00;
    if (device.chip_family == CF_P16F_A) cmd = 0x0a;
    if (device.chip_family == CF_P16F_D) cmd = 0x0d;
	...
	if (cmd != 0x00) {
		...
		putByte(0x01);
		putByte(num);
		getByte(cmd | 0x80); // wait for response
		for (i = 0; i < num; i++) *data++ = getByte(0);
		return 1;
	} else
		return 0;
...
int p16a_program_config(void) {
    ...
    if ((device.chip_family==CF_P16F_B) |
