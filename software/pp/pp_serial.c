...
void initSerialPort(void)
    ...
    timeout_sets.ReadIntervalTimeout         = 1; // MAXDWORD; //1;
    timeout_sets.ReadTotalTimeoutMultiplier  = 1000; //MAXDWORD; //1000;
    timeout_sets.ReadTotalTimeoutConstant    = 1; //MAXDWORD - 10; //1;
...
/*
    if (!SetCommMask(port_handle, EV_RXCHAR)) {
        printf("events settings failed\n");
        CloseHandle(port_handle);
        clean_exit(0);
    }

    ovEvent.hEvent = CreateEvent(
...
int getByte(unsigned char wait_for) {
    ...
    // clear buffer on entry
    buf[0] = buf[1] = 0;

//    this for async mode communication
//    if (WaitCommEvent(port_handle, (LPDWORD)((void *)&comm_event), &ovEvent)) {
//        ResetEvent(ovEvent.hEvent);
    if(wait_for != 0) {
        do {
            ReadFile(port_handle, buf, 1, (LPDWORD)((void *)&n), NULL);
        } while(buf[0] != wait_for);
    } else {
        ReadFile(port_handle, buf, 1, (LPDWORD)((void *)&n), NULL);
    }
        ...
        comErr("Serial port failed to receive a byte, read returned %d\n", n);
//    } else {
//        comErr("No data receives on com port\n");
//    }
